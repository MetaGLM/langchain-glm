...
async def dialogue_page(
    ...
    if prompt:
        ...
        named_generators = [
            ("chat", agent_executor.invoke(chat_input=prompt))
        ]
        ...
        tasks: Dict[asyncio.Task, Tuple[str, AsyncIterator, int, Dict]] = {
            asyncio.create_task(get_next_chunk(generator)): (step_name, generator, message_id, chat_box)
        ...
        while tasks:
            ...
            for task in completed_tasks:
                (step_name, generator, msg_id, c_box) = tasks.pop(task)
                try:
                    chunk = AddableDict({step_name: task.result()})
                    run_ui(msg_id, chunk, c_box)
                    new_task = asyncio.create_task(get_next_chunk(generator))
                    tasks[new_task] = (step_name, generator, msg_id, c_box)
                except StopAsyncIteration:
                    pass


def run_ui(message_id, chunk, chat_box):
    metadata = {
        "message_id": message_id,
    }
    item = chunk.get("chat")
